...
`let` 和 `var` 之间有些微妙的差别，但目前对于我们来说并不重要。我们将会在 [var](/var) 章节中介绍它们。
...
- `let` — 现代的变量声明方式。
- `var` — 老旧的变量声明方式。一般情况下，我们不会再使用它。但是，我们会在 [var](/var) 章节介绍 `var` 和 `let` 的微妙差别，以防你需要它们。
...
<https://zh.javascript.info/variables#tasks>
