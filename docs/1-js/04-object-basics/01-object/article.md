...
```js 
// Math.tc 是内置的去除小数部分的方法。
alert( String(Math.tc(Number("49"))) ); // "49"，相同，整数属性
alert( String(Math.tc(Number("+49"))) ); // "49"，不同于 "+49" ⇒ 不是整数属性
alert( String(Math.tc(Number("1.2"))) ); // "1"，不同于 "1.2" ⇒ 不是整数属性
```
...
<https://zh.javascript.info/object#tasks>
